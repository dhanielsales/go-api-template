...
func TestDeleteCategory(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fields: &fields{
				controller: newCategoryController(t, func(mocks *mocks) {
					mocks.categoryService.EXPECT().DeleteCategory(gomock.Any(), gomock.Any()).Return(int64(0), errors.New("error"))
				}),
			},
			args: &args{
		...
		},
		{
			...
			fields: &fields{
				controller: newCategoryController(t, func(mocks *mocks) {
					mocks.categoryService.EXPECT().DeleteCategory(gomock.Any(), gomock.Any()).Return(int64(1), nil)
				}),
			},
			args: &args{
				c: testutils.NewEchoContext(ctx, nil).WithParam("id", "c7fe4f9f-f3f0-49ef-861b-66072d72c197"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := tt.fields.controller.DeleteCategory(tt.args.c)
