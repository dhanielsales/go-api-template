...
func TestUpdateCategory(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: &args{
				c: testutils.
					NewEchoContext(ctx, []byte(`{`)).WithParam("id", "c7fe4f9f-f3f0-49ef-861b-66072d72c197"),
			},
			expected: &expected{
		...
		{
			name: "Error updating one category",
			fields: &fields{
				controller: newProductController(t, func(mocks *mocks) {
					mocks.productService.EXPECT().UpdateProduct(gomock.Any(), gomock.Any(), gomock.Any()).Return(int64(0), errors.New("error"))
				}),
			},
			args: &args{
				c: testutils.
					NewEchoContext(ctx, []byte(`{ "name": "name", "description": "description" }`)).
					WithParam("id", "c7fe4f9f-f3f0-49ef-861b-66072d72c197"),
			},
			expected: &expected{
		...
		},
		{
			...
			fields: &fields{
				controller: newProductController(t, func(mocks *mocks) {
					mocks.productService.EXPECT().UpdateProduct(gomock.Any(), gomock.Any(), gomock.Any()).Return(int64(1), nil)
				}),
			},
			args: &args{
				c: testutils.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := tt.fields.controller.UpdateProduct(tt.args.c)
